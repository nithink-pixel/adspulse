...
DB_PATH = os.path.join(BASE_DIR, "data", "adspulse.duckdb")
...
st.set_page_config(
    page_title="Uber Ads Strategy Review OS",
    page_icon="📊",
    layout="wide",
...
# ── Sidebar ───────────────────────────────────────────────────────────────────
st.sidebar.image("https://upload.wikimedia.org/wikipedia/commons/c/cc/Uber_logo_2018.png", width=100)
st.sidebar.markdown("## Ads Strategy Review OS")
st.sidebar.markdown("---")
...
with tab3:
    ...
    st.markdown("*12 validation rules enforced on every ETL run. Bad records quarantined before reaching dashboards.*")
    st.markdown("---")
